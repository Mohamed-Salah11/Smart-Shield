...
from app.config import _ss_dir as _ss_dir_ovpn
...
_OPENVPN_HOOK = os.path.join(_ss_dir_ovpn("/usr/local/etc"), "openvpn-hook.sh")
...
# ---------------------------------------------------------------------------
# Lifecycle hook script (client-connect / client-disconnect)
# ---------------------------------------------------------------------------

_HOOK_SCRIPT_TEMPLATE = """#!/bin/sh
# Smart Shield — OpenVPN lifecycle hook
# Auto-generated by app/services/openvpn_writer.py — DO NOT EDIT MANUALLY
#
# OpenVPN invokes this script with one positional argument:
#   $1  path to a temp file containing client metadata (client-connect only)
# and a rich environment (common_name, trusted_ip, trusted_port, bytes_sent,
# bytes_received, time_duration, script_type=client-connect|client-disconnect).
#
# We POST a compact JSON event to the Smart Shield admin app over loopback
# so the connection shows up in the central audit log / SIEM pipeline.

URL="http://127.0.0.1:5000/vpn/api/openvpn/hook"
SECRET="__SECRET__"

EVENT="${script_type:-unknown}"
BODY=$(printf '{"event":"%s","cn":"%s","trusted_ip":"%s","trusted_port":"%s","bytes_sent":"%s","bytes_received":"%s","duration":"%s","instance":"%s"}' \\
    "$EVENT" "$common_name" "$trusted_ip" "$trusted_port" \\
    "$bytes_sent" "$bytes_received" "$time_duration" "$SMARTSHIELD_INSTANCE")

# HMAC-SHA256 of the body under the shared secret — verified server-side.
SIG=$(printf '%s' "$BODY" | openssl dgst -sha256 -hmac "$SECRET" -hex 2>/dev/null | awk '{print $NF}')

# Fire-and-forget; never block the daemon if the admin app is unreachable.
/usr/local/bin/curl -fsS --max-time 2 \\
    -H "Content-Type: application/json" \\
    -H "X-SmartShield-Sig: $SIG" \\
    -X POST -d "$BODY" "$URL" >/dev/null 2>&1 || true

exit 0
"""


def _openvpn_hook_secret(conn) -> str:
    """Get or mint the HMAC secret used by openvpn-hook.sh -> /vpn/api/openvpn/hook."""
    import secrets
    try:
        row = conn.execute(
    ...
    try:
        conn.execute(
            ...
            "VALUES ('openvpn_hook_secret', ?, CURRENT_TIMESTAMP) "
...
def install_openvpn_hook_script(conn) -> dict:
    """Write the SmartShield OpenVPN client-connect/disconnect hook with mode 0750.

    Idempotent: re-running with the same secret leaves the script untouched.
    Returns ``{"ok": bool, "path": str, "message": str}``.
    """
    if not _on_freebsd():
        return {"ok": True, "path": _OPENVPN_HOOK, "message": "skipped-non-freebsd"}
    try:
        secret = _openvpn_hook_secret(conn)
        body   = _HOOK_SCRIPT_TEMPLATE.replace("__SECRET__", secret)
        os.makedirs(os.path.dirname(_OPENVPN_HOOK), exist_ok=True)
        try:
            with open(_OPENVPN_HOOK, "r", encoding="utf-8") as fh:
                if fh.read() == body:
                    return {"ok": True, "path": _OPENVPN_HOOK,
                            "message": "already up-to-date"}
        except OSError:
            pass
        from app.services.config_file_utils import atomic_write
        atomic_write(_OPENVPN_HOOK, body, mode=0o750)
        return {"ok": True, "path": _OPENVPN_HOOK, "message": "installed"}
    except Exception as exc:
        return {"ok": False, "path": _OPENVPN_HOOK, "message": str(exc)}
...
def generate_server_conf(row: dict) -> str:
    ...
    # v29 columns — match migration defaults so legacy rows keep working.
    dh_length    = _v(row, "dh_parameter_length", "2048")
    ecdh_curve   = _v(row, "ecdh_curve", "default")
    data_ciphers = _v(row, "data_encryption_algorithms", "AES-256-GCM:AES-128-GCM:AES-256-CBC")
    fallback     = _v(row, "fallback_data_encryption_algorithm", "AES-256-CBC")
    auth_dig     = _v(row, "auth_digest_algorithm", "SHA256")
    verbosity    = row.get("verbosity_level") if row.get("verbosity_level") is not None else 3
    ping_method  = _v(row, "ping_method", "keepalive").lower()
    ...
    L = [
        ...
        f"dh   {_OPENVPN_KEYS}/dh{dh_length}.pem",
    ]
    if ecdh_curve and ecdh_curve.lower() != "default":
        L.append(f"ecdh-curve {ecdh_curve}")
    if use_tls:
    ...
    if ping_method == "ping":
        L += [f"ping {ping_int}", f"ping-restart {ping_tout}"]
    else:
        L.append(f"keepalive {ping_int} {ping_tout}")
    L += [
    ...
    # Lifecycle hooks — every client connect/disconnect POSTs into the
    # Smart Shield audit log via openvpn-hook.sh so the events show up in
    # the SOC feed alongside IDS alerts, eligible for correlation/risk.
    L += [
        "",
        "# Lifecycle hooks (Smart Shield SIEM)",
        f"client-connect    {_OPENVPN_HOOK}",
        f"client-disconnect {_OPENVPN_HOOK}",
        "script-security 2",
        f'setenv SMARTSHIELD_INSTANCE "{sid}"',
    ]
...
def write_openvpn_configs(conn) -> dict:
    ...
    install_openvpn_hook_script(conn)
...
def apply_openvpn(conn) -> dict:
    ...
    servers  = _rows(conn, "SELECT * FROM openvpn_servers WHERE disabled=0 ORDER BY id")
    clients  = _rows(conn, "SELECT * FROM openvpn_clients WHERE disabled=0 ORDER BY id")
    # If nothing is enabled, stop the service and clear its rc.conf knob so
    # the global reload-all does not leave a stale tunnel running.
    if not servers and not clients:
        if not _on_freebsd():
            return {"ok": True, "skipped": True,
                    "message": "OpenVPN has no enabled servers/clients — non-FreeBSD, nothing to stop.",
                    "configs": []}
        from app.services.service_manager import service_action, sysrc_set
        sysrc_set("openvpn_enable", "NO")
        service_action("openvpn", "stop")
        return {"ok": True, "skipped": True,
                "message": "OpenVPN has no enabled servers/clients — openvpn stopped and disabled in rc.conf.",
                "configs": []}
    all_errs = []
    for row in servers:
