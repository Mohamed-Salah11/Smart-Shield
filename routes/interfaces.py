...
def get_interface_groups():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT id, group_name, group_description, group_members FROM interface_groups')
...
def save_interface_group():
    try:
        ...
        conn = sqlite3.connect('data.db')
        cursor = conn.cursor()
        cursor.execute('''INSERT INTO interface_groups (group_name, group_description, group_members) 
                          VALUES (?, ?, ?)''', 
...
def get_interface_group(group_id):
    try:
        conn = sqlite3.connect('data.db')
        cursor = conn.cursor()
        cursor.execute('SELECT id, group_name, group_description, group_members FROM interface_groups WHERE id = ?', (group_id,))
...
def update_interface_group(group_id):
    try:
        ...
        conn = sqlite3.connect('data.db')
        cursor = conn.cursor()
        cursor.execute('''UPDATE interface_groups SET group_name = ?, group_description = ?, group_members = ? WHERE id = ?''',
